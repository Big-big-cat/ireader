<template><!--header.html-->
  <header id="header" class="container mb-4">
    <nav class="navbar navbar-light">
      ...
      <nav v-if="!this.$store.state.islogin" id="signout" class="my-2 my-md-0 mr-md-3 small">
        <a class="p-1 text-muted" href="register.html">注册</a>
        <b class="p-1 text-muted" >|</b>
        <router-link class="p-1 text-muted" to="/login">登录</router-link>
      </nav>
      <nav v-else id="signin" class="my-2 my-md-0 mr-md-3 small">
        ...
        <a class="p-1 text-muted" href="#">Welcome <span id="uname">{{this.$store.state.uname}}</span></a>
...
<script>
  export default {
    methods:{
      signout(){
        this.axios.get(
          "http://localhost:3000/users/signout"
        ).then(res=>{
